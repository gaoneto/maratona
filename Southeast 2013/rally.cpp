...
vector<pair<int, pair<int, int> > > travel[1111];
int vis[1800][555], d[1800][555];
...
void dijkstra(int t, int u)
{
    ...
    while(!q.empty())
    {
        ...
        if(vis[t][u]) continue;
        vis[t][u]=1;
        if(t<240 && d[t+1][u]>d[t][u]+2)
        {
            d[t+1][u]=d[t][u]+2;
            q.push(make_pair(-d[t+1][u], make_pair(t+1, u)));
        }
        if(t>=240 && t+1<1800 && d[t+1][u]>d[t][u]+1)
        {
            d[t+1][u]=d[t][u]+1;
            q.push(make_pair(-d[t+1][u], make_pair(t+1, u)));
        }
        for(i=0; i<adj[u].size(); i++)
        {
            ...
            for(j=0; j<travel[id].size(); j++)
            {
                ...
                if(min(t, 240)>=tempo && d[min(t, 240)-tempo][v]>d[t][u]+tempo)
                {
                    d[min(t, 240)-tempo][v]=d[t][u]+tempo;
                    q.push(make_pair(-d[min(t, 240)-tempo][v], make_pair(min(t, 240)-tempo, v)));
                }
            }
        }
    }
...
int main()
{
    ...
    {
        for(i=0; i<1800; i++) for(j=0; j<n; j++) d[i][j]=INF, vis[i][j]=0;
        ...
        dijkstra(240, 0);
        int ans=INF;
        for(i=0; i<1800; i++) ans=min(ans, d[i][n-1]);
        printf("%d\n", ans-720);
    }
    return 0;
}
