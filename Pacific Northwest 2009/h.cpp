...
int dp[N][N], dplu[N][N], dpru[N][N], dplb[N][N], dprb[N][N], s[N], n, m;
char x[N][N];
...
{
    int i;
    for(i=1; i<=m; i++) s[i]=sum(a, i, b, i);
}

//Build the array for Kadane's algorithm consisting of interval from column a to column b inclusively
void build_column(int a, int b)
{
    int i;
    for(i=1; i<=n; i++) s[i]=sum(i, a, i, b);
}

int main()
{
    int i, j, k, v, soma, maxrow, maxcolumn;
