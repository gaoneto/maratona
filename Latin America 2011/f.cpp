...
int main()
{
    int i, j, f, pos, curlcp;
    long long curset;
    while(scanf("%d", &f) && f)
    {
        s="";
        while(!st.empty()) st.pop();
        st.push(make_pair(0LL, -1));
        sub.clear();
        N=curlcp=curset=0;
        for(i=0; i<f; i++)
        {
        ...
        lcp(s);
        LCP[N-1]=0;
        for(i=f; i<N; i++) if(len[id[SA[i]]]==dis[SA[i]] && LCP[i]<len[id[SA[i]]] && LCP[i-1]<len[id[SA[i]]]) sub.insert(1LL << id[SA[i]]);
        for(i=f; i<N; i++)
        {
            if(!curlcp)
            {
            ...
            {
                if(LCP[i]==curlcp) curset |= (1LL << id[SA[i]]);
                else if(LCP[i]>curlcp)
                {
                    st.push(make_pair(curset, curlcp));
                    curlcp=LCP[i];
                    curset=0LL;
                    curset |= (1LL << id[SA[i]]);
                }
                else
                {
                    ...
                    sub.insert(curset);
                    curlcp = LCP[i];
                    while(st.top().second>=LCP[i])
                    {
                        curset |= st.top().first;
                        if(st.top().second>LCP[i]) sub.insert(curset);
                        st.pop();
                    }
                    if(!LCP[i]) curset=0LL;
                }
